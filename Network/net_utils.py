...
from Network import attention_transfer
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if down_times > 1:
            self.downsample = attention_transfer.Downsample(in_channels, inner_channels, down_times)

        self.attn_layers = nn.Sequential(
            *[
                attention_transfer.LocalAttnLayer(
                    self.inner_channels,
                    self.patch_size,
                    num_heads,
                    dropout,
